...
AI 不应直接执行 shell 命令修改系统环境（如 export PATH），而应输出 结构化的环境描述，由系统安全地应用。Env Engine 正是为此设计：将环境配置抽象为 Env Patch，实现会话隔离、可回滚、可持久化确认。
...
· 支持 diff 预览 和 用户确认 后才将环境持久化到系统配置文件（如 .bashrc），并提供回滚机制
